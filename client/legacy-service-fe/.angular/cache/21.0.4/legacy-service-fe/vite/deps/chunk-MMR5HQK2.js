...
function updateCustomControl(tNode, lView, control, modelName) {
  ...
  const state2 = control.state();
  const bindings = getControlBindings(lView);
  maybeUpdateInput(directiveDef, directive, bindings, state2, CONTROL_VALUE, modelName);
  for (const key of CONTROL_BINDING_KEYS) {
    const inputName = CONTROL_BINDING_NAMES[key];
    maybeUpdateInput(directiveDef, directive, bindings, state2, key, inputName);
  }
  if (tNode.flags & 8192) {
    updateNativeControl(tNode, lView, control);
  }
}
function maybeUpdateInput(directiveDef, directive, bindings, state2, key, inputName) {
...
function updateNativeControl(tNode, lView, control) {
  ...
  const name = state2.name();
  if (controlBindingUpdated(bindings, NAME, name)) {
    renderer.setAttribute(element, "name", name);
  }
  updateBooleanAttribute(renderer, element, bindings, state2, DISABLED);
  updateBooleanAttribute(renderer, element, bindings, state2, READONLY);
  updateBooleanAttribute(renderer, element, bindings, state2, REQUIRED);
  if (tNode.flags & 16384) {
    updateOptionalAttribute(renderer, element, bindings, state2, MAX);
    updateOptionalAttribute(renderer, element, bindings, state2, MIN);
  }
  if (tNode.flags & 32768) {
    updateOptionalAttribute(renderer, element, bindings, state2, MAX_LENGTH);
    updateOptionalAttribute(renderer, element, bindings, state2, MIN_LENGTH);
  }
}
function updateBooleanAttribute(renderer, element, bindings, state2, key) {
  const value = state2[key]();
  if (controlBindingUpdated(bindings, key, value)) {
...
function updateOptionalAttribute(renderer, element, bindings, state2, key) {
  const value = state2[key]?.();
  if (controlBindingUpdated(bindings, key, value)) {
    const name = CONTROL_BINDING_NAMES[key];
...
//# sourceMappingURL=chunk-MMR5HQK2.js.map
