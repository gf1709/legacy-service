Set-Location -Path (Split-Path -Path $MyInvocation.MyCommand.Definition -Parent)

function echo_error {
    Write-Host $args[0] -BackgroundColor Red
}

function echo_ok {
    Write-Host $args[0] -BackgroundColor Blue
}

$branches = git branch -a | Where-Object { $_ -match 'remotes' -and $_ -notmatch 'HEAD' }
$grepResultFile = %temp%\grepResult.txt
foreach ($branch in $branches) {
    $b = $branch -replace 'remotes/origin/', ''
    $b = $b.Trim()
    if ($b -eq 'master') {
        echo_error "ignore master !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
        continue
    }
    # echo_ok($branch)
    # echo_ok('current branch is ', $b)
    $searchString = '\bif\b'
    git checkout $b
    $grepResult = Get-ChildItem -Recurse -Include *.java | Select-String -Pattern  $searchString
    if ($grepResult) {
        "found----------------------------------" | Out-File -FilePath $grepResultFile -Append -Encoding utf8
        $b | Out-File -FilePath $grepResultFile -Append -Encoding utf8
        $grepResult | Out-File -FilePath $grepResultFile -Append -Encoding utf8
        "sector end-----------------------------" | Out-File -FilePath $grepResultFile -Append -Encoding utf8
        ""
    }
}
# Get-ChildItem -Recurse -Include *.java | Select-String -Pattern 'if'